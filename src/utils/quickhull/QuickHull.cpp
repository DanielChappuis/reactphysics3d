...
PolygonVertexArray* QuickHull::computeConvexHull(uint32 nbPoints, const void* pointsStart,
    ...
    Map<const QHHalfEdgeStructure::Face*, Array<uint32>> mapFaceToRemainingClosestPoints(allocator);

    QHHalfEdgeStructure convexHullHalfEdgeStructure(allocator);

    Map<const QHHalfEdgeStructure::Face*, Vector3> mapFaceToNormal(allocator);

    Array<const QHHalfEdgeStructure::Face*> initialFaces(allocator);

    // Compute the initial convex hull
    computeInitialHull(points, convexHullHalfEdgeStructure, mapFaceToNormal,
                       mapFaceToRemainingClosestPoints, initialFaces, orphanPointsIndices, allocator);
    ...
    associateOrphanPointsToNewFaces(orphanPointsIndices, initialFaces, points, mapFaceToNormal,
                                    mapFaceToRemainingClosestPoints);

    // TODO : DELETE THIS
    std::cout << showMap(mapFaceToRemainingClosestPoints) << std::endl;

    // Get The next vertex candidate
    uint32 nextVertexIndex;
    QHHalfEdgeStructure::Face* nextFace;
    findNextVertexCandidate(mapFaceToRemainingClosestPoints, mapFaceToNormal, points, nextVertexIndex, nextFace);
    while (nextVertexIndex != INVALID_VERTEX_INDEX) {
        ...
        addVertexToHull(nextVertexIndex, nextFace, points, mapFaceToNormal, convexHullHalfEdgeStructure,
                        mapFaceToRemainingClosestPoints, allocator);
...
void QuickHull::addVertexToHull(uint32 vertexIndex,
    ...
    findHorizon(points[vertexIndex], face, mapFaceToNormal, points, allocator, horizonVertices, visibleFaces);
    ...
    deleteVisibleFaces(visibleFaces, convexHullHalfEdgeStructure, mapFaceToRemainingClosestPoints,
                       mapFaceToNormal, orphanPointsIndices, horizonVertices, allocator);

    // Build the new faces
    Array<const QHHalfEdgeStructure::Face*> newFaces(allocator);
    buildNewFaces(vertexIndex, horizonVertices, convexHullHalfEdgeStructure, mapFaceToNormal, mapFaceToRemainingClosestPoints, points, newFaces, allocator);

    associateOrphanPointsToNewFaces(orphanPointsIndices, newFaces, points, mapFaceToNormal, mapFaceToRemainingClosestPoints);
}
...
void QuickHull::buildNewFaces(uint32 newVertexIndex,
    ...
    for (uint32 i=0; i < horizonVertices.size(); i += 2) {
        ...
        // Create a new face
        QHHalfEdgeStructure::Face* face = convexHullHalfEdgeStructure.addFace(faceVertices);

        newFaces.add(face);

        Vector3 faceNormal = (points[v3->externalIndex] - points[v2->externalIndex]).cross(points[v->externalIndex] - points[v3->externalIndex]);
        faceNormal.normalize();
        mapFaceToNormal.add(Pair<const QHHalfEdgeStructure::Face*, Vector3>(face, faceNormal));
        mapFaceToRemainingClosestPoints.add(Pair<const QHHalfEdgeStructure::Face*, Array<uint32>>(face, Array<uint32>(allocator)));
...
void QuickHull::deleteVisibleFaces(const Array<QHHalfEdgeStructure::Face*>& visibleFaces,
    ...
    for (uint32 i=0; i < nbFaces; i++) {
        ...
        auto it = mapFaceToRemainingClosestPoints.find(visibleFaces[i]);
        assert(it != mapFaceToRemainingClosestPoints.end());
        orphanPoints.addRange(it->second);
        ...
        // Remove the face from the map of faces to remaining closest points
        mapFaceToRemainingClosestPoints.remove(it);

        // Remove the face from the map of faces to normal
        mapFaceToNormal.remove(visibleFaces[i]);
    ...
    while (facesToVisit.size() > 0) {
        ...
        do {
            ...
            if (!visitedFaces.contains(nextFace)) {
                ...
                if (mapFaceToNormal[nextFace].dot(vertex - faceVertex) > 0) {
...
void QuickHull::findNextVertexCandidate(Map<const QHHalfEdgeStructure::Face*, Array<uint32>>& mapFacesToRemainingClosestPoints,
                                        Map<const QHHalfEdgeStructure::Face*, Vector3>& mapFacesToNormal,
                                        Array<Vector3>& points,
                                        uint32& outNextVertexIndex,
    ...
    Array<uint32>* maxFaceArray = nullptr;

    // For each face
    for (auto it = mapFacesToRemainingClosestPoints.begin(); it != mapFacesToRemainingClosestPoints.end(); ++it) {

        // If the face has remaining candidates points
        if (it->second.size() > 0) {

            std::cout << "Face with vertices " << it->first->verticesString() << " has candidates points" << std::endl;

            Vector3& faceNormal = mapFacesToNormal[it->first];
            Vector3& faceVertex = points[it->first->getVertex()->externalIndex];
            ...
            for (uint32 i=0; i < it->second.size(); i++) {
                ...
                if (distance > maxDistance) {
                    ...
                    maxVertexI = i;
                    outNextFace = const_cast<QHHalfEdgeStructure::Face*>(it->first);
                    maxFaceArray = &(it->second);
                }
            }
        }
    }
    ...
    if (outNextFace != nullptr) {
        assert(maxFaceArray != nullptr);
        maxFaceArray->removeAt(maxVertexI);
    }
...
void QuickHull::associateOrphanPointsToNewFaces(Array<uint32>& orphanPointsIndices,
    ...
    for (uint32 i=0; i < orphanPointsIndices.size(); i++) {
        ...
        findClosestFaceForVertex(orphanPointsIndices[i], newFaces, points, mapFaceToNormal, mapFaceToRemainingClosestPoints);
    }
}

// Find the closest face for a given vertex and add this vertex to the remaining closest points for this face
void QuickHull::findClosestFaceForVertex(uint32 vertexIndex, Array<const QHHalfEdgeStructure::Face*>& faces, Array<Vector3>& points,
                                         Map<const QHHalfEdgeStructure::Face*, Vector3>& mapFaceToNormal,
                                         Map<const QHHalfEdgeStructure::Face*, Array<uint32>>& mapFaceToRemainingClosestPoints) {

    decimal minDistanceToFace = DECIMAL_LARGEST;
    const QHHalfEdgeStructure::Face* closestFace = nullptr;

    // For each new face
    for (uint32 f=0; f < faces.size(); f++) {

        const QHHalfEdgeStructure::Face* face = faces[f];
        const QHHalfEdgeStructure::Vertex* v1 = face->getVertex();

        const Vector3& normal = mapFaceToNormal[face];

        const decimal distanceToFace = normal.dot(points[vertexIndex] - points[v1->externalIndex]);
    ...
    if (closestFace != nullptr) {
        ...
        mapFaceToRemainingClosestPoints[closestFace].add(vertexIndex);
    }
}
...
void QuickHull::computeInitialHull(Array<Vector3>& points, QHHalfEdgeStructure& convexHullHalfEdgeStructure,
    ...
    const QHHalfEdgeStructure::Face* face0 = nullptr;
    const QHHalfEdgeStructure::Face* face1 = nullptr;
    const QHHalfEdgeStructure::Face* face2 = nullptr;
    const QHHalfEdgeStructure::Face* face3 = nullptr;
    ...
    if (planeNormal.dot(v1V4) < 0) {
        ...
        face0 = convexHullHalfEdgeStructure.addFace(face1Vertices);
        face1 = convexHullHalfEdgeStructure.addFace(face2Vertices);
        face2 = convexHullHalfEdgeStructure.addFace(face3Vertices);
        face3 = convexHullHalfEdgeStructure.addFace(face4Vertices);

        mapFaceToNormal.add(Pair<const QHHalfEdgeStructure::Face*, Vector3>(face0, planeNormal));
        mapFaceToNormal.add(Pair<const QHHalfEdgeStructure::Face*, Vector3>(face1, (points[i3] - points[i2]).cross(points[i4] - points[i2])));
        mapFaceToNormal.add(Pair<const QHHalfEdgeStructure::Face*, Vector3>(face2, (points[i2] - points[i1]).cross(points[i4] - points[i1])));
        mapFaceToNormal.add(Pair<const QHHalfEdgeStructure::Face*, Vector3>(face3, (points[i4] - points[i1]).cross(points[i3] - points[i1])));
    }
    else {
        ...
        mapFaceToNormal.add(Pair<const QHHalfEdgeStructure::Face*, Vector3>(face0, -planeNormal));
        mapFaceToNormal.add(Pair<const QHHalfEdgeStructure::Face*, Vector3>(face1, (points[i4] - points[i2]).cross(points[i3] - points[i2])));
        mapFaceToNormal.add(Pair<const QHHalfEdgeStructure::Face*, Vector3>(face2, (points[i4] - points[i1]).cross(points[i2] - points[i1])));
        mapFaceToNormal.add(Pair<const QHHalfEdgeStructure::Face*, Vector3>(face3, (points[i3] - points[i1]).cross(points[i4] - points[i1])));
    }

    mapFaceToRemainingClosestPoints.add(Pair<const QHHalfEdgeStructure::Face*, Array<uint32>>(face0, Array<uint32>(allocator)));
    mapFaceToRemainingClosestPoints.add(Pair<const QHHalfEdgeStructure::Face*, Array<uint32>>(face1, Array<uint32>(allocator)));
    mapFaceToRemainingClosestPoints.add(Pair<const QHHalfEdgeStructure::Face*, Array<uint32>>(face2, Array<uint32>(allocator)));
    mapFaceToRemainingClosestPoints.add(Pair<const QHHalfEdgeStructure::Face*, Array<uint32>>(face3, Array<uint32>(allocator)));
