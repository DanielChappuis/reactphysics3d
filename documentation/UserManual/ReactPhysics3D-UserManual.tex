...
CapsuleShape* capsuleShape = physicsCommon.createCapsuleShape(1.0, 2.0);
  \end{lstlisting}
    ...
    To use this method, you first need to create an array of \texttt{PolygonFace} to describe each face of your mesh. You also need to have an array with
    ...
    to instantiate a \texttt{ConvexMesh} using your \texttt{PolygonVertexArray}. Now that you have a \texttt{ConvexMesh}, you can create the
    \texttt{ConvexMeshShape}

    The following example shows how to create a convex mesh shape when you know the vertices and faces of your mesh.
...
PolygonVertexArray::IndexDataType::INDEX_INTEGER_TYPE);
  ...
  \end{lstlisting}
    ...
    There is a second way to create a \texttt{ConvexMesh}. In case you only know the vertices of your mesh (you don't know the faces), you can use the
    following method:
...
vertices[21] = -3; vertices[22] = 3; vertices[23] = -3;
  ...
  assert(convexMesh != nullptr);
    ...
    will be scaled from the origin by this factor when used in the collision shape. It means that you can use the same \texttt{ConvexMesh} for
  ...
  with \texttt{ConcaveMeshShape} is quite expensive compared to convex shapes. Note collsions between two concave mesh shape are not supported. \\

  In order to create a concave mesh shape, we first need to create a \texttt{TriangleMesh}. A \texttt{TriangleMesh} describes your mesh made of triangles
  and can be reused multiple times to instantiate \texttt{ConcaveMeshShapes} with different scaling factors for instance. 
...
if (messages.size() > 0) {
  ...
  \texttt{Concave\allowbreak MeshShape}. Note that you can use the same \texttt{TriangleMesh} instance for creating multiple
  \texttt{ConcaveMeshShape} with a different scaling factor each time. \\
  \end{sloppypar}
  ...
  First, you need to create an \texttt{HeightField}. To do that, you need to have an array with all the height values of your field.
  You can have height values of type integer, float or double. You need to specify the number of rows and columns of your two
